...
.dash-subtitle { color:rgba(245,194,0,0.85) !important; font-size:13px; margin-top:4px; font-weight:400; }
.dash-bee      { font-size:40px; line-height:1; }

.filter-bar {
    background: #1a2434; border-radius: 12px; padding: 14px 20px;
    border: 1px solid rgba(255,255,255,0.08); margin-bottom: 18px;
}

.chart-card {
    background: #1a2434; border-radius: 14px; padding: 18px 20px 12px;
    border: 1px solid rgba(255,255,255,0.07);
    box-shadow: 0 4px 20px rgba(0,0,0,0.25); margin-bottom: 16px;
}

.sec-hdr {
    font-size:13px; font-weight:600; color:rgba(255,255,255,0.9) !important;
...
.kpi-spark-label {
    ...
    letter-spacing: 0.8px; color: rgba(255,255,255,0.45) !important; margin-bottom: 6px;
}
.kpi-spark-value {
...
.kpi-spark-value.white { color: #FFFFFF !important; }

.kpi-delta-row {
    font-size: 12px; font-weight: 600; display: flex; align-items: center; gap: 6px;
}
.delta-up-good   { color: #34D399 !important; }
.delta-down-good { color: #34D399 !important; }
.delta-up-bad    { color: #F87171 !important; }
.delta-down-bad  { color: #F87171 !important; }
...
.aging-badge {
    background:rgba(245,194,0,0.12); border:1px solid rgba(245,194,0,0.35); border-radius:6px;
    padding:5px 12px; font-size:11px; color:#F5C200 !important; font-weight:600;
...
.styled-table th { padding:9px 11px; text-align:center;
                   font-weight:600; font-size:11px; color:rgba(255,255,255,0.85) !important;
                   text-transform:uppercase; letter-spacing:0.5px; border-bottom:1px solid rgba(245,194,0,0.2); }
...
.styled-table td { padding:8px 11px; text-align:center;
                   border-bottom:1px solid rgba(255,255,255,0.05); color:rgba(255,255,255,0.8) !important; }
.styled-table .col-date { text-align:left; font-weight:600; color:rgba(245,194,0,0.9) !important; }
.styled-table .col-high { color:#F87171 !important; font-weight:700; }

.appendix-card {
    background: #1a2434; border-radius:14px; padding:22px 26px;
    border:1px solid rgba(255,255,255,0.07); margin-bottom:8px;
}
.appendix-title { font-size:15px; font-weight:700; color:#F5C200 !important; margin-bottom:14px; }
.appendix-row { display:flex; gap:8px; margin-bottom:10px; align-items:flex-start; }
.appendix-key { font-size:12px; font-weight:600; color:rgba(255,255,255,0.9) !important; min-width:220px; }
.appendix-val { font-size:12px; color:rgba(255,255,255,0.55) !important; }
.appendix-formula { font-family:'DM Mono',monospace; font-size:11px;
                    background:rgba(245,194,0,0.08); border:1px solid rgba(245,194,0,0.2);
                    border-radius:5px; padding:2px 8px; color:#F5C200 !important; display:inline-block; margin-top:3px; }
...
_AX = dict(
    gridcolor="rgba(255,255,255,0.06)", linecolor="rgba(255,255,255,0.10)",
    tickcolor="rgba(255,255,255,0.15)", showgrid=True,
    tickfont=dict(color="rgba(255,255,255,0.55)", size=11),
    title_font=dict(color="rgba(255,255,255,0.7)"),
)
_BASE = dict(
    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    font=dict(color="rgba(255,255,255,0.8)", family="DM Sans, sans-serif", size=11),
    legend=dict(bgcolor="rgba(26,36,52,0.9)", bordercolor="rgba(255,255,255,0.1)",
                borderwidth=1, font=dict(size=11, color="rgba(255,255,255,0.8)")),
    hoverlabel=dict(bgcolor="#1a2434", bordercolor="#F5C200", font_color="#FFFFFF", font_size=12),
...
def _trend_color(values, lower_is_better=True):
    ...
    return "#34D399" if improving else "#F87171"
...
def _spark_kpi(col_w, number, label, value_str, val_class, spark_svg, delta_val, delta_label, lower_is_better=True):
    d_fid  = delta_val
    ...
    <div class="kpi-spark">
      ...
      <div style="font-size:10px; color:rgba(255,255,255,0.25); margin-top:2px;">↑ last 7 days</div>
...
fid_cls = "green" if d_fid_v <= 0 else "red"
bl_cls  = "green" if d_bl_v  <= 0 else "red"
zt_cls  = "amber"

with kc1:
    _spark_kpi(kc1, 1, "Total In-Process (FID)",
               f"{tot_fid:,.0f}", fid_cls,
               _sparkline_svg(spark_fid, fid_color),
               d_fid_v, "vs prev day", lower_is_better=True)

with kc2:
    _spark_kpi(kc2, 2, "Overall Backlog",
               f"{overall_bl:,.0f}", bl_cls,
               _sparkline_svg(spark_bl, bl_color),
               d_bl_v, "FID+RID vs prev", lower_is_better=True)

with kc3:
    zt_display = f"{int(zt_val):,}" if zt_val > 0 else "0"
    _spark_kpi(kc3, 3, "Zone Transfer Parcels",
               zt_display, zt_cls,
               _sparkline_svg(spark_zt, zt_color),
               d_zt_v, "vs prev day", lower_is_better=False)
...
def _pct_kpi(col_w, number, label, value, prev_value, lower_is_better=True):
    ...
    v_color = "#34D399" if good else ("#F87171" if diff != 0 else "#FFFFFF")
    ...
    <div class="kpi-small">
      ...
      <div style="font-size:32px; font-weight:700; font-family:'DM Mono',monospace;
                  color:{v_color}; margin-bottom:10px; line-height:1;">{value:.2f}%</div>
      <div class="kpi-delta-row">
        <span class="{d_cls}" style="font-size:18px; line-height:1;">{arr}</span>
...
with col_sort:
    ...
    if lt_fr is not None:
        ...
        _layout(fig10, height=280, extra={
            "showlegend": False,
            "xaxis": dict(**_AX, title="Count"),
            "yaxis": dict(**_AX),
    ...
    if not ft_range.empty:
        ...
        if "Total In Progress (Backlog)" in ft_range.columns:
            fig_track.add_trace(go.Bar(
                ...
                name="Total In Process (FID)", marker_color=C_ISD, opacity=0.85,
            ))
        if "Worked On" in ft_range.columns:
            fig_track.add_trace(go.Bar(
                x=ft_range["Date_Label"], y=ft_range["Worked On"],
                name="Worked On", marker_color=C_SUB, opacity=0.85,
            ))
...
with col_region:
    ...
    if tot_fid > 0:
        ...
        fig_reg.update_traces(
            ...
            textfont=dict(color="rgba(255,255,255,0.9)", size=13),
            width=0.45,
        )
        ...
        st.markdown(f"""
        <div style="display:flex; gap:16px; justify-content:center; padding:6px 0 4px;">
          <div style="text-align:center;">
            <div style="font-size:10px; color:rgba(255,255,255,0.35); text-transform:uppercase; letter-spacing:0.6px; margin-bottom:2px;">ISD</div>
            <div style="font-size:22px; font-weight:700; color:{C_ISD}; font-family:'DM Mono',monospace;">{isd_total:,.0f}</div>
          </div>
          <div style="text-align:center;">
            <div style="font-size:10px; color:rgba(255,255,255,0.35); text-transform:uppercase; letter-spacing:0.6px; margin-bottom:2px;">OSD</div>
            <div style="font-size:22px; font-weight:700; color:{C_OSD}; font-family:'DM Mono',monospace;">{osd_total:,.0f}</div>
          </div>
          <div style="text-align:center;">
            <div style="font-size:10px; color:rgba(255,255,255,0.35); text-transform:uppercase; letter-spacing:0.6px; margin-bottom:2px;">Total</div>
            <div style="font-size:22px; font-weight:700; color:{C_AMB}; font-family:'DM Mono',monospace;">{tot_fid:,.0f}</div>
          </div>
        </div>""", unsafe_allow_html=True)
    else:
...
with col_aging:
    ...
    if not ag_f.empty:
        ...
        if not ag_melt.empty:
            ...
            fig8.update_traces(
                textposition="outside", textfont=dict(color="rgba(255,255,255,0.8)", size=9),
...
with st.expander("📖 Appendix — Definitions & Calculation Methods", expanded=False):
    ...
    items = [
        ("1. Total In-Process (FID)",
         "All parcels currently in the First Inbound Delivery process on the selected date. Green = count is lower than yesterday (improving).",
         "Aging_Distribution → ISD Total + OSD Total"),
        ("2. Overall Backlog (FID+RID)",
         "Total undelivered backlog across FID and RID. Green = total is falling.",
         "FID Backlog + RID Backlog  (Source: FID_RID_Backlog_Details)"),
        ("3. Zone Transfer Parcels",
         "Parcels transferred between delivery zones on the selected date. Sparkline shows 7-day movement.",
         "Dashboard_Card → 'Zone Transfer' column"),
        ("4. Backlog — FID vs RID (Donut)",
         "Visual split of overall backlog between First Inbound (FID) and Return Inbound (RID).",
         "FID Backlog and RID Backlog from FID_RID_Backlog_Details"),
        ("5. FID Backlog %",
         "Percentage of total in-process FID parcels in backlog. Green arrow + green value = % is lower than yesterday.",
         "FID Backlog ÷ Total In-Process × 100"),
        ("6. Zone Change %",
         "Share of total parcels that underwent a zone transfer. Green = lower than yesterday.",
         "Zone Transfer ÷ Total (Dashboard_Card) × 100"),
        ("7. Backlog Details — FID & RID (LMH/FMH)",
         "Stacked bar breakdown by service type and region. LMH = Last Mile Hub, FMH = First Mile Hub.",
         "FID_RID_Backlog_Details → FID/RID × LMH/FMH × ISD/SUB/OSD columns"),
        ("8. Sort — FID Sort vs RID Sort",
        ...
        ("9. Date-wise Backlog Progress Tracking",
         "Grouped bar chart: Total In-Process FID (blue) vs Worked On (green) over a custom date range. Use the From/To pickers above the chart.",
         "FID_Tracking → 'Total In Progress (Backlog)' and 'Worked On'"),
        ("10. Region Wise In-Process Parcels",
         "Vertical bars comparing ISD vs OSD parcel counts on the selected report date.",
         "Aging_Distribution → ISD Total and OSD Total"),
        ("11. Aging Distribution",
         "Percentage of parcels per day-bucket (1d → 10+d) by region. Threshold: ISD/SUB ≥ 4 days, OSD ≥ 5 days = aged.",
         "Aging_Distribution — columns 1 through 10+"),
        ("Sparkline Colors",
         "Green sparkline = last value is better than the day before. Red = worsening. Covers last 7 available data points.",
         "Current day vs previous day comparison"),
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown(f"""
<div style="text-align:center; color:rgba(255,255,255,0.2); font-size:11px; padding:20px 0 10px; letter-spacing:0.5px;">
  🐝 Carrybee Delivery Intelligence &nbsp;·&nbsp;
  Report Date: <span style="color:rgba(245,194,0,0.5);">{sel_date.strftime('%d %b %Y')}</span>
