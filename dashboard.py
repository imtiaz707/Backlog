...
def load_data():
    ...
    for col in ["Newly_Added", "Total_In_Progress", "Worked_On", "Carry_Forward"]:
        if col in ft.columns:
            ft[col] = pd.to_numeric(ft[col].astype(str).str.replace(',', ''), errors="coerce").fillna(0)
    ...
    for c in age_cols + ["Total"]:
        if c in ag.columns: 
            ag[c] = pd.to_numeric(ag[c].astype(str).str.replace(',', ''), errors="coerce").fillna(0)
...
with f_col1:
    ...
    if all_dates:
        ...
        sel_start = pd.Timestamp.now()
        sel_end = pd.Timestamp.now()
...
prev_dc   = dc_f.sort_values("Date").iloc[-2] if len(dc_f) > 1 else None
...
kpi_data = [
    ...
    (k2, "FID Backlog %", "bg-green",
     ...
     delta_html(latest_dc["FID_Backlog_Pct"] if latest_dc is not None else None, prev_dc["FID_Backlog_Pct"] if prev_dc is not None else None, fmt="{:.2%}")),
