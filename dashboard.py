...
.kpi-card {
    ...
    color: #ffffff; /* White text for bright backgrounds */
}
...
/* ---- sidebar ---- */
.sidebar-title { font-size: 20px; font-weight: 700; color: #e6edf3; margin-bottom: 4px; }
...
def load_data():
    ...
    dc.rename(columns={
        "Date": "Date", "ISD": "ISD", "OSD": "OSD", "Total": "Total",
        "FID Backlog (%)": "FID_Backlog_Pct", "Zone Transfer": "Zone_Transfer"
    }, inplace=True)
    ...
    ft.rename(columns={
        "Report Date": "Report_Date", "Newly Added": "Newly_Added",
        "Total In Progress (Backlog)": "Total_In_Progress",
        "Worked On": "Worked_On", "Carry Forward": "Carry_Forward",
    }, inplace=True)
...
with col_title:
    st.markdown("## Logistics Operations Dashboard")
with col_date:
...
# ── KPI Row (Updated to match target image colors) ────────────────────────────
k1, k2, k3, k4, k5 = st.columns(5)

kpi_data = [
    (k1, "Total In Process", "bg-blue",
...
# ── Row 2: Combo Bar/Line Chart + Donut Chart ─────────────────────────────────
c_combo, c_donut = st.columns([3, 2])

with c_combo:
    st.markdown('<div class="section-header">Daily Backlog Workflow</div>', unsafe_allow_html=True)

    fig_combo = go.Figure()
    # Add Bars for Total In Progress and Newly Added
    ...
    # Add Line for Worked On (if toggled)
    if show_worked:
    ...
    # Group bars side by side instead of stacking
    fig_combo.update_layout(**PLOT_THEME, height=360, barmode='group')
    fig_combo.update_xaxes(title_text="")
    fig_combo.update_yaxes(title_text="Count")
    st.plotly_chart(fig_combo, use_container_width=True)

with c_donut:
    st.markdown('<div class="section-header">Current Backlog Distribution</div>', unsafe_allow_html=True)

    if latest_dc is not None:
        labels = ['ISD', 'OSD']
        values = [latest_dc['ISD'], latest_dc['OSD']]
        
        fig_donut = go.Figure(data=[go.Pie(
            labels=labels, 
            values=values, 
            hole=.6, # Creates the donut shape
        ...
        # Move legend to bottom center to match the visual reference
...
with c_aging:
    st.markdown('<div class="section-header">Aging Distribution (Most Recent Date)</div>', unsafe_allow_html=True)
    ...
    if not ag_melt.empty:
        ...
        fig_age.update_xaxes(title_text="")
...
with c_region:
    ...
    fig_reg.update_xaxes(title_text="")
    fig_reg.update_yaxes(title_text="")
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown(
    "<p style='text-align:center;color:#484f58;font-size:12px;padding-top:20px'>"
    "Courier Logistics Dashboard • Carrybee Intelligence"
    "</p>",
    unsafe_allow_html=True,
)
