...
.dash-title    { color:#FFFFFF !important; font-size:28px; font-weight:700; margin:0; letter-spacing:-0.5px; }
.dash-subtitle { color:rgba(245,194,0,0.85) !important; font-size:14px; margin-top:4px; font-weight:400; }
...
/* ── Section Headers ── */
.sec-hdr {
    font-size:14px; font-weight:700; color:rgba(255,255,255,0.95) !important;
...
.aging-badge {
    ...
    padding:6px 14px; font-size:12px; color:#F5C200 !important; font-weight:600;
...
_AX = dict(
    ...
    tickfont=dict(color="rgba(255,255,255,0.55)", size=12),
    title_font=dict(color="rgba(255,255,255,0.7)"),
)
_BASE = dict(
    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    font=dict(color="rgba(255,255,255,0.85)", family="DM Sans, sans-serif", size=12),
    legend=dict(bgcolor="rgba(26,36,52,0.9)", bordercolor="rgba(255,255,255,0.1)",
                borderwidth=1, font=dict(size=12, color="rgba(255,255,255,0.85)")),
    hoverlabel=dict(bgcolor="#1a2434", bordercolor="#F5C200", font_color="#FFFFFF", font_size=13),
...
with kc1:
    _spark_kpi(kc1, 1, "Total In-Process (FID)", f"{tot_fid:,.0f}", fid_cls, _sparkline_svg(spark_fid, fid_color), d_fid_v, "vs prev day", True)

with kc2:
    _spark_kpi(kc2, 2, "Overall Backlog", f"{overall_bl:,.0f}", bl_cls, _sparkline_svg(spark_bl, bl_color), d_bl_v, "FID+RID vs prev", True)
...
with col_region:
    with st.container(border=True):
        ...
        if tot_fid > 0:
            ...
            st.markdown(f"""
            <div style="display:flex; gap:16px; justify-content:center; padding:12px 0 4px; border-top: 1px solid rgba(255,255,255,0.1);">
              <div style="text-align:center; flex:1; border-right: 1px solid rgba(255,255,255,0.1);">
                <div style="font-size:12px; color:rgba(255,255,255,0.4); text-transform:uppercase; letter-spacing:0.8px; margin-bottom:4px;">ISD</div>
                <div style="font-size:26px; font-weight:700; color:{C_ISD}; font-family:'DM Mono',monospace;">{isd_total:,.0f}</div>
              </div>
              <div style="text-align:center; flex:1; border-right: 1px solid rgba(255,255,255,0.1);">
                <div style="font-size:12px; color:rgba(255,255,255,0.4); text-transform:uppercase; letter-spacing:0.8px; margin-bottom:4px;">OSD</div>
                <div style="font-size:26px; font-weight:700; color:{C_OSD}; font-family:'DM Mono',monospace;">{osd_total:,.0f}</div>
              </div>
              <div style="text-align:center; flex:1;">
                <div style="font-size:12px; color:rgba(255,255,255,0.4); text-transform:uppercase; letter-spacing:0.8px; margin-bottom:4px;">Total</div>
                <div style="font-size:26px; font-weight:700; color:{C_AMB}; font-family:'DM Mono',monospace;">{tot_fid:,.0f}</div>
              </div>
            </div>""", unsafe_allow_html=True)
        else:
    ...
    with st.container(border=True):
        ...
        if not ag_f.empty:
            ...
            if not ag_melt.empty:
                ...
                fig8.update_traces(
                    textposition="outside", textfont=dict(color="rgba(255,255,255,0.85)", size=11),
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown(f"""
<div style="text-align:center; color:rgba(255,255,255,0.3); font-size:12px; padding:20px 0 10px; letter-spacing:0.5px;">
  🐝 Carrybee Delivery Intelligence &nbsp;·&nbsp;
  Report Date: <span style="color:rgba(245,194,0,0.6);">{sel_date.strftime('%d %b %Y')}</span>
