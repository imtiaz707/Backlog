...
st.set_page_config(
    ...
    page_icon="📦",
    layout="wide",
    initial_sidebar_state="collapsed", # Collapsed to focus on the horizontal layout
)
...
with col_date:
    latest_ft_date = ft.sort_values("Report_Date").iloc[-1] if not ft.empty else None
    if latest_ft_date is not None:
        st.markdown(f"<p style='text-align:right;color:#8b949e;padding-top:12px'>As of <b style='color:#e6edf3'>{latest_ft_date['Report_Date'].strftime('%d %b %Y')}</b></p>", unsafe_allow_html=True)

st.markdown("---")

# ── TOP LAYER: Horizontal Filters (From Sketch) ───────────────────────────────
...
with f_col3:
    st.markdown("<br>", unsafe_allow_html=True) # Spacer to push toggles down slightly
    t_col1, t_col2 = st.columns(2)
    with t_col1: show_worked = st.toggle("Show Worked On", value=True)
    with t_col2: show_carry  = st.toggle("Show Carry Forward", value=False)
...
# ── SECOND LAYER: 4 KPIs (From Sketch) ─────────────────────────────────────────
