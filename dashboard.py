...
/* ── DELTA BADGES ── */
.kpi-delta-row { font-size: 12px; font-weight: 600; }
...
/* ── SECTION HEADERS ── */
.sec-hdr {
    font-size: 12px; font-weight: 700; color: #1C2B3A !important;
...
.aging-badge {
    ...
    font-size: 11px; color: #8A6A00 !important;
...
_AX = dict(
    ...
    showgrid=True,
    tickfont=dict(color="#6B7E91", size=11, family="DM Mono"),
    title_font=dict(color="#6B7E91"),
)
_BASE = dict(
    ...
    legend=dict(
        ...
        borderwidth=1,
        font=dict(size=11, color="#1C2B3A"),
    ),
    hoverlabel=dict(
        bgcolor="#1C2B3A", bordercolor="#F5C200",
        font_color="#F0EDE5", font_size=12,
    ),
...
def _spark_kpi(col_w, num, label, val_str, val_cls, spark_html, delta, note="", lower_is_better=True):
    ...
    <div class="kpi-spark">
      ...
      {f'<div style="font-size:10px;color:{C_SLATE};margin-top:4px;">{note}</div>' if note else ''}
      <div style="font-size:9px;color:{C_SLATE};margin-top:5px;font-family:DM Mono,monospace;opacity:0.6;">7-day trend</div>
    ...
    with st.container(border=True):
        ...
        if (fid_bl + rid_bl) > 0:
            ...
            fig_dn.add_annotation(
                ...
                font=dict(size=13, color=C_NAVY), xanchor="center",
            )
            _layout(fig_dn, height=175,
                    extra={"margin": dict(l=0,r=0,t=0,b=0),
                           "legend": dict(orientation="h", yanchor="bottom", y=-0.22,
                                          xanchor="center", x=0.5,
                                          font=dict(size=10, color=C_NAVY))})
...
def _pct_kpi(col_w, num, label, val, prev, lower_is_better=True, note=""):
    ...
    <div class="kpi-small">
      <div class="kpi-spark-label">{num} · {label}</div>
      <div style="font-size:36px;font-weight:800;font-family:'DM Mono',monospace;
            ...
            for lbl, tot in zip(labs, tots):
                if tot > 0:
                    fig7.add_annotation(x=tot, y=lbl, text=f"  <b>{tot:,.0f}</b>",
                                        ...
                                        font=dict(size=12, color=C_NAVY))
    ...
    with st.container(border=True):
        ...
        if lt_fr is not None:
            ...
            fig8 = go.Figure(data=[go.Bar(
                ...
                textfont=dict(size=13, color=C_NAVY, family="DM Mono"),
                width=0.45,
            )])
    ...
    with st.container(border=True):
        ...
        if tot_fid > 0:
            ...
            fig10.update_traces(
                ...
                textfont=dict(color=C_NAVY, size=13, family="DM Mono"),
                width=0.45)
    ...
    with st.container(border=True):
        ...
        if not ag_f.empty:
            ...
            if not ag_melt.empty:
                ...
                fig11.update_traces(
                    textposition="outside",
                    textfont=dict(color=C_NAVY, size=9),
...
# ── FOOTER ───────────────────────────────────────────────────────────────────
st.markdown(f"""
<div style="text-align:center;color:{C_SLATE};font-size:11px;
